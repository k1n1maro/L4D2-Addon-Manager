<div align="center">

# L4D2 Addon Manager

![Version](https://img.shields.io/github/v/release/k1n1maro/L4D2-Addon-Manager?style=for-the-badge&logo=github&color=ff6b35&labelColor=2d3748)
![Downloads](https://img.shields.io/github/downloads/k1n1maro/L4D2-Addon-Manager/total?style=for-the-badge&logo=download&color=38a169&labelColor=2d3748)
![Release Date](https://img.shields.io/github/release-date/k1n1maro/L4D2-Addon-Manager?style=for-the-badge&logo=calendar&color=805ad5&labelColor=2d3748)
![License](https://img.shields.io/github/license/k1n1maro/L4D2-Addon-Manager?style=for-the-badge&color=f56565&labelColor=2d3748)
![Platform](https://img.shields.io/badge/Platform-Windows_10/11-0078d4?style=for-the-badge&logo=windows&labelColor=2d3748)
![Language](https://img.shields.io/badge/Language-Python_3.8+-3776ab?style=for-the-badge&logo=python&labelColor=2d3748)       

**🇷🇺 Программа для управления модами в Left 4 Dead 2**
*Включайте и выключайте моды одним кликом. Работает с любой версией игры!*

**🇺🇸 Program for managing mods in Left 4 Dead 2**
*Turn mods on and off with one click. Works with any game version!*

![Главный интерфейс](screenshots/Screenshot_4.png)

</div>

<div align="center">

## Что умеет программа?

</div>

### 🇷🇺 Для русских игроков
- **Включать и выключать моды** - Просто нажмите на переключатель рядом с модом
  (Принудительное изменение gameinfo. Позволяеть включать моды на VERSUS серверах)
- **Работает с любой игрой** - Steam версия, пиратка - без разницы
- **Сама обновляется** - Нажали кнопку "Обновить" и все готово
- **Поиск модов** - Введите название и найдете нужный мод
- **Можете переименовать** - Нажмите правой кнопкой и измените название

### 🇺🇸 For English players
- **Turn mods on/off** - Just click the switch next to any mod
  (Forced change of gameinfo. Allows you to enable mods on VERSUS servers)
- **Works with any game** - Steam version, pirated - doesn't matter
- **Updates itself** - Click "Update" button and you're done
- **Search mods** - Type name and find what you need
- **Can rename** - Right-click and change the name

<details>
<summary>📸 <strong>Больше скриншотов</strong></summary>

<div align="center">

### Главное меню
![Главное меню](screenshots/Screenshot_1.png)

### Настройки программы
![Настройки](screenshots/Screenshot_3.png)

### Информация о модах
![Информация](screenshots/Screenshot_4.png)

### Процесс сканирования
![Сканирование](screenshots/Screenshot_5.png)

### Дополнительные функции
![Функции](screenshots/Screenshot_6.png)

### Интерфейс в действии
![Действие](screenshots/Screenshot_7.png)

</div>

</details>

<div align="center">

## 📥 Скачать программу

[![Download EXE](https://img.shields.io/badge/💾_СКАЧАТЬ_EXE-v1.3.0-success?style=for-the-badge&logo=download)](https://github.com/k1n1maro/L4D2-Addon-Manager/releases/latest/download/L4D2_Addon_Manager_v1.3.0.exe)

</div>

<div align="center">

## 🚀 Как начать пользоваться

### Шаг 1: Скачайте программу
### Шаг 2: Укажите папку с игрой
### Шаг 3: Готово!

</div>

<div align="center">

## 💝 Поддержать проект

[![Boosty](https://img.shields.io/badge/💎_Boosty-k1n1maro-ff6854?style=for-the-badge&logo=heart)](https://boosty.to/k1n1maro)
[![DonationAlerts](https://img.shields.io/badge/🔔_DonationAlerts-k1n1maro-00d4aa?style=for-the-badge&logo=paypal)](https://www.donationalerts.com/r/k1n1maro)

**Карта Сбербанка:** `2202 2067 3893 4277`

</div>

<div align="center">

## 📝 История обновлений

| Версия | Дата | Что нового |
|--------|------|------------|
| **v1.3.0** | 16.12.2024 | 🚀 Теперь все моды показывают названия, программа открывается в 5 раз быстрее, обновления работают автоматически|
| **v1.2.0** | 13.12.2024 | 🔧 Исправили обновления, добавили английский язык, стало стабильнее работать |
| **v1.1.0** | 13.12.2024 | 🐛 Исправили ошибки при первом запуске |
| **v1.0.0** | 10.12.2024 | 🎉 Первая версия программы |

</div>

<div align="center">

## 🐛 Есть проблемы?

[![Issues](https://img.shields.io/badge/🐛_Сообщить_о_проблеме-GitHub-red?style=for-the-badge&logo=github)](https://github.com/k1n1maro/L4D2-Addon-Manager/issues/new)

## 👨💻 Автор

**k1n1maro**
[![GitHub](https://img.shields.io/badge/GitHub-k1n1maro-181717?style=flat&logo=github)](https://github.com/k1n1maro)
[![Steam](https://img.shields.io/badge/Steam-kinimaro-1b2838?style=flat&logo=steam)](https://steamcommunity.com/id/kinimaro/) 

---

⭐ **Поставьте звезду, если программа понравилась!**

*Сделано с ❤ для игроков Left 4 Dead 2*

</div>
